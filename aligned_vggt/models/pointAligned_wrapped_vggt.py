...
class VGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, images: torch.Tensor, num_overlap: int, context: dict = None, gt_poses: torch.Tensor = None) -> dict:
        """
        Forward pass of the point-aligned VGGT model.
        Args:
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            if self.point_head is not None:
                ...
                if context is not None:
                    ...
                    context_overlapping_point_maps = context["world_points"][-1][:, -num_overlap:] # (B, N_overlap, H, W, 3)
                    context_overlapping_point_confidences = context["world_points_conf"][-1][:, -num_overlap:] # (B, N_overlap, H, W, 3)
                    ...
                    alignment_transform = torch.stack(batch_transforms) # (B, 4, 4)
            ...
            if self.camera_head is not None:
                ...
                if self.point_head is not None:
                    ...
                    # apply transform                    
                    aligned_extr = apply_sim3_alignment_on_w2c(extr,alignment_transform,batch_scales)    
...
def weighted_umeyama_sim3(
    ...
    """
    Weighted Umeyama algorithm for SIM(3) estimation.
    Args:
        src: (M,3) source points
        dst: (M,3) destination points
        weights: (M,) per-point weights
    Returns:
...
def irls_sim3_umeyama(
    ...
    conf_threshold_factor: float = 0.5,
    delta: float = 0.1,
    max_iters: int = 20,
    tol: float = 1e-9,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Iteratively Reweighted Least Squares (IRLS) for robust SIM(3) estimation using weighted Umeyama.
    Args:
        src: (N, H, W, 3) source point maps
        dst: (N, H, W, 3) destination point maps
        conf_src: (N, H, W) source point confidence maps
        conf_dst: (N, H, W) destination point confidence maps
        conf_threshold_factor: factor to multiply median confidence for adaptive thresholding
        delta: Huber loss delta parameter
        max_iters: maximum number of IRLS iterations
