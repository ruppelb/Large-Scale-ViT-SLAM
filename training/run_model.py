...
class StepProgressBar(TQDMProgressBar):
    """
    Custom progress bar for training steps.
    Instead of showing epoch progress, it shows total training steps.
    """
...
class CustomModelCheckpoint(ModelCheckpoint):
    """
    Custom ModelCheckpoint that saves the latest checkpoint to a specified path and appends rank to checkpoint filenames in DDP.
    Fixes some issues with Lightning's built-in ModelCheckpoint in distributed settings.
    Overrides checkpoint saving at end of epoch and validation.
    """
...
class DynamicDataModule(L.LightningDataModule):
    """
    Dynamic DataModule that instantiates datasets based on the provided configuration.
    Sets the seed for each dataset to ensure reproducibility.

    Args:
        data_cfg (Dict[str, Any]): Configuration dictionary for datasets.
        seed (int): Seed value for dataset initialization.
    """
    def __init__(
        self,
    ...
    def on_before_batch_transfer(self, batch, dataloader_idx):
        ...
        with torch.amp.autocast("cuda", enabled=False):
            # we only convert quantities to be expressed in first frame coordinate frame here, no rescaling or normalization is done
...
class LitModel(L.LightningModule, PyTorchModelHubMixin):
    """
    LightningModule for training and evaluating the model.
    Loads model from checkpoint or pretrained weights, sets up loss, optimizer, and metrics.
    Freezes specified modules during training.

    Args:
        cfg (Dict[str, Any]): Configuration dictionary for the model, loss, optimizer, and metrics.
    """
    def __init__(self, cfg):
        ...
        if cfg.checkpoint.model_checkpoint_path is not None:
            ...
            self._load_model_checkpoint(cfg.checkpoint.model_checkpoint_path, cfg.checkpoint.fallback_checkpoint_path if "fallback_checkpoint_path" in cfg.checkpoint else None)
        else:
            self.model = instantiate({"_target_": cfg.model._target_}, _recursive_=False).from_pretrained(cfg.checkpoint.from_pretrained,cache_dir=cfg.checkpoint.save_dir)
    ...
    def forward(self, batch: dict):
        ...
        # perform gt alignment if necessary and convert to tensors
    ...
    def _load_model_checkpoint(self, ckpt_path: str, fallback_ckpt_path: str = None):
        ...
        Args:
            ckpt_path (str): Path to the main checkpoint file.
            fallback_ckpt_path (str, optional): Path to the fallback checkpoint file for filling missing keys.
        """
        ...
        # try to fill missing keys from fallback checkpoint
...
parser = argparse.ArgumentParser(description= "Run large-scale VIT SLAM training or inference based on the given config. Supports distributed training.")
